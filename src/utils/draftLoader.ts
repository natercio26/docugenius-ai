...
export const INVENTARIO_TEMPLATE = `ESCRITURA PÚBLICA DE INVENTÁRIO E PARTILHA, na forma abaixo:
...
/**
 * Loads draft data from sessionStorage, converting it into a proper Draft object
 * @param isNew Whether this is a new draft
...
export const loadDraftData = (isNew: boolean): Draft | null => {
  ...
  if (!draftFromSession) return null;
  ...
  try {
    ...
    // Verificar texto de qualificação no sessionStorage
    ...
    throw new Error('Erro ao carregar o rascunho');
  }
};

/**
 * Prepares extracted data to pass to the DraftViewer component
 * @param draft The draft object
 * @returns The prepared extracted data
 */
export const prepareDraftData = (draft: Draft): Record<string, string> => {
  ...
  return extractedDataToPass;
};

/**
 * Saves the updated draft to sessionStorage
 * @param draft The draft to save
 * @param title The new title for the draft
